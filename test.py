...
from StackAutoEncoder.model import Model
...
def cos_sim(vec1, vec2):
    num = np.matmul(vec1, vec2.T)
    denom = np.linalg.norm(vec1) * np.linalg.norm(vec2)
    cos = num/denom
    sim = 0.5 + 0.5 * cos  # 相似度归一化 相似度都是归一化到[0,1]区间内，因此余弦相似度表示为 cosineSIM = 0.5cosθ + 0.5
    return sim
...
def main():
    ...
    model = Model(config, N, dims, X_target, X_node)
