...
class SNE(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():#, tf.device('/cpu:0'):
            ...
            # Variables.
            network_weights = self._initialize_weights()#初始化权值
            ...
            #主要是选取一个张量里面索引对应的元素。tf.nn.embedding_lookup（tensor, id）:tensor就是输入张量，id就是张量对应的索引
            self.attr_embed =  tf.matmul(self.train_data_attr, self.weights['attr_embeddings'])  # batch_size * attr_dim
            self.embed_layer = tf.concat(1, [self.id_embed, self.alpha * self.attr_embed]) # batch_size * (id_dim + attr_dim) 连接两个矩阵的操作，第一个参数代表连接的维度
    ...
    def _initialize_weights(self):#初始化权值
        all_weights = dict()
        all_weights['in_embeddings'] = tf.Variable(tf.random_uniform([self.node_N, self.id_embedding_size], -1.0, 1.0))  # id_N * id_dim 产生-1~1之间的均匀分布向量
        ...
        all_weights['out_embeddings'] = tf.Variable(tf.truncated_normal([self.node_N, self.id_embedding_size + self.attr_embedding_size],
                                    stddev=1.0 / math.sqrt(self.id_embedding_size + self.attr_embedding_size)))#截断的正态分布
